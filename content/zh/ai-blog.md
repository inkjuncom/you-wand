...
花了大概 60 块钱，买了一个满意的域名。整个过程我花了不到十分钟——搜索、加入购物车、付款、验证邮箱。Cloudflare 的界面干净利落，没有 GoDaddy 那种「加 99 元买隐私保护」的套路。

域名到手的那一刻，我有了一种「这是我自己的地方」的感觉。
...
我用 Tailwind CSS 做样式，接入了霞鹜文楷做手写字体。设计上我选了「暮山紫」配色——暖紫底、紫罗兰点缀，想要那种温暖又克制的杂志感。

文章我用 Markdown 来写，构建时通过 `gray-matter` + `remark` 编译成 JSON，配合 `shiki` 做代码高亮。因为要部署到 Cloudflare Workers，运行时没有 Node 文件系统，所以必须预编译。国际化我也自己写了一套轻量的方案，中英文双切。

一个周末下来，站点的基本功能就有了：首页文章列表、详情页、中英切换、暗色模式。
...
我的想法是：**把所有功能和设计原封不动地搬过去，但换成 TanStack Start 的底层。**
